...
class Solution:
    def getRow(self, rowIndex: int) -> List[int]:
        ...
        # we know that given rowIndex, we should return C(rowIndex)(1) , ... , C(rowIndex)(rowIndex)
        # nCr format
        ...
        if rowIndex%2 == 0: ans[halfIndex+1:] = reversed(ans[:halfIndex])
        else: ans[halfIndex+1:] = reversed(ans[:halfIndex+1])
        return ans
